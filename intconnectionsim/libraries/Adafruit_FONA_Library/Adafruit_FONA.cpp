...
bool Adafruit_FONA::begin(Stream &port) {
  ...
  return (!sendCheckReply(F("AT+CPMS=" FONA_PREF_SMS_STORAGE "," FONA_PREF_SMS_STORAGE
                   "," FONA_PREF_SMS_STORAGE),
                 F("NORMAL POWER DOWN"))) && (!sendCheckReply(F("AT+CPMS=" FONA_PREF_SMS_STORAGE "," FONA_PREF_SMS_STORAGE
                   "," FONA_PREF_SMS_STORAGE),
                 F("ERROR")));
#endif
...
bool Adafruit_FONA::callPhone(char *number) {
  ...
  //bool res =  sendCheckReply(replybuffer, "NO DIALTONE");
  DEBUG_PRINTLN("second");
  DEBUG_PRINTLN(replybuffer);
  return (!sendCheckReply(sendbuff, "NO DIALTONE")) && (!sendCheckReply(sendbuff, "NORMAL POWER DOWN")) && (!sendCheckReply(sendbuff, "ERROR"));
